...
export class Barrier extends Actor {
	...
	touched: boolean;
	barrierIndex: number;
	constructor(initalPos: Point, bw = 100, angle: number, actor: Actor, index: number) {
		super(initalPos);
	...
	update(delta: number): void {
		...
		let distance = Math.sqrt(Math.pow(barrierPos.x - actorPos.x, 2) + Math.pow(barrierPos.y - actorPos.y, 2));

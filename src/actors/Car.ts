...
export class Car extends Actor {
	...
	constructor(initialPos: Point, keyboardMap: KeyboardMap, size: Size = { w: 50, h: 100 }) {
		super(initialPos);
		this.carSize = size;
		this.carColor = 'red';
		this.angle = 0;
	...
	update(delta: number): void {
		...
		let newPos: Point = {
			x: this.position.x + Math.cos(converAngleToRad(this.angle)) * this.carSpeed,
			y: this.position.y + Math.sin(converAngleToRad(this.angle)) * this.carSpeed,
		};
